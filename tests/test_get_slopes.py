...
@pytest.mark.parametrize(
    "test_input,expected",
    [
        ("calorimetry_data_1.csv", 8.0e-8),
        ("calorimetry_data_2.csv", 8.5e-8),
    ],
)
def test_get_average_slope(test_input, expected):
    ...
    assert "gradient_of_mean_slope" in average_slopes.columns
    assert round(average_slopes.at[0, "gradient_of_mean_slope"], 9) == expected
